    ...
    try {
      ...
      if (newUserToken.isPresent()) {
        String resMessage = String
            .format("User token with email: %s has been successfully added to the user table",
                userToken.getUserId());
        logger.info(resMessage);
      } else {
        throw new Exception(
            String.format("Error in creating a user token with email: %s", userToken.getUserId()));
      }
