...
public class SurveyQuestionsServiceImpl implements SurveyQuestionsService {
    ...
    @Override
    public Optional<SurveyQuestionsDetails> getSurveyQuestionsDetails(Long surveyId) throws Exception {
        ...
        try {
            ...
            if (surveyQuestionsDTO.isPresent()
                ...
                Map<Integer, String> questionTypeMap = questionTypeList.get().stream()
                        .collect(Collectors.toMap(QuestionType::getQuestionTypeID, QuestionType::getQuestionTypeText));
                ...
                String resMessage = String.format("Survey: %s with question details :%s successfully build.", surveyId, surveyQuestionsDetails.get());
                logger.info(resMessage);
            } else {
            ...
            String errMessage = String.format("Survey: %s or its questions does not exists", surveyId);
            logger.error(errMessage);
            e.printStackTrace();
            throw e;
        }
