...
public class QuestionDAOImpl implements QuestionDAO {
  ...
  public Optional<Question> getQuestionById(long question_id) throws Exception {
    ...
    try(Connection connection = ConnectionManager.getInstance().getDBConnection().get();
        ...
        PreparedStatement stmt_choiceQuestions = connection
                .prepareStatement(getQuestionOptions_query)){
      ...
      while (resultSet.next()){
        ...
        if(questionTypeId == 2 || questionTypeId == 3){
          stmt_choiceQuestions.setLong(1, question_id);
          ResultSet resultSetOptions = stmt_choiceQuestions.executeQuery();
          while (resultSetOptions.next()){
            optionTextList.add(resultSetOptions.getString("option_text"));
            optionValueList.add(resultSetOptions.getInt("option_value"));
            if(resultSetOptions.isLast()){
              rows = resultSetOptions.getRow();
            }
          }
