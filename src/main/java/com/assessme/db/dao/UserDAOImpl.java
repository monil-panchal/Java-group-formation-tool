    ...
    try (
      ...
      if (row > 0) {
        ...
        String failureString = String
            .format("Failed to insert the user with email: %s record in the DB", user.getEmail());
  ...
  public Optional<User> updateUserPassword(User user) throws Exception {
    ...
    try (
      ...
      if (row > 0) {
        ...
        String failureString = String
            .format("Failed to update the user with email: %s record in the DB", user.getEmail());
        logger.error(failureString);
        throw new Exception(failureString);
      }
