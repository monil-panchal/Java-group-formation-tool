...
public class UserPasswordHistoryServiceImplTest {
    ...
    private Logger logger = LoggerFactory.getLogger(UserPasswordHistoryServiceImplTest.class);
    ...
    public void addPasswordModificationRecordTest() throws Exception {
        ...
        Mockito.when(userPasswordHistoryDAO.addPasswordModificationRecord(userPasswordHistory)).thenReturn(optionalUserPasswordHistory);

        passwordHistory = userPasswordHistoryService.addUserPasswordRecord(userPasswordHistory);

        Assert.isTrue(passwordHistory.isPresent(), "User password history record should not be empty");
